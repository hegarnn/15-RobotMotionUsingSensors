...
def wait_for_seconds(n):
    ...
    start = time.time()
    current = time.time()
    while (current - start >= n) == False:
        current = time.time()
    print('Gooodbye')

def wait_for_inches(n):
...
class SimpleRoseBot(object):
    ...
    def go(self, rspeed, lspeed):
        self.lmotor.turn_on(lspeed)
        self.rmotor.turn_on(rspeed)
    ...
    def go_straight_for_seconds(self,time,speed):
        self.rmotor.turn_on(speed)
        self.lmotor.turn_on(speed)
        self.stop(time)
